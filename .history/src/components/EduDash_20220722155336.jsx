...
const EduDash = () => {
  ...
  const [ethRevenue, setEthRevenue] = useState({});
  ...
  useEffect(() => {
    async function getEthRevenue() {
      try {
        const Revenue = Moralis.Object.extend("Withdrawal");
        const revenueQuery = new Moralis.Query(Revenue);
        revenueQuery.equalTo("address", user.attributes.accounts[0]);
        const educatorRevenue = await revenueQuery.find();
        console.log(educatorRevenue)
      } catch(error) {
        console.log(error)
      }
    }
