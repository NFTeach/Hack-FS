...
import { Card, Button, List, Modal, notification } from "antd";
...
const SubmitTest = (props) => {
  ...
  useEffect(() => {
    if (!isLoading && !isFetching && data) {
      ...
      Modal.success({
        title:
          "Congrats! Your test is being stored on smart contract and will be available soon for students!",
        content: (
          <div>
            <p>
  ...
  const uploadTestToIpfsAndSetTokenMetadata = async () => {
    ...
    if (isAuthenticated) {
      notification.info({
        ...
        description:
          "Your test is being upload to IPFS and data stored on the smart contract. PLEASE DON'T REFRESH PAGE!",
      });
      ...
      if (formattedData) {
        ...
        const file = new Moralis.File("file.json", {
          base64: btoa(JSON.stringify(dataFile)),
        });
  ...
  if (error) {
    notification.error({
      message: "Error",
      description:
        "Please try again and make sure you are using a valid educator wallet address",
    });
    setIsUploadInProgress(false);
  }
